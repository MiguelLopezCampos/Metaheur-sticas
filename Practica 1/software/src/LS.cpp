...
///////////////////////////////////Flip
int flip(vector <bool> & sol, vector <bool> & rep)
{
	bool valido = false;
	int tam = sol.size();
	int r;
	...
	vector <Caracteristicas> & test)
{
	...
	while(mejora)
	{
		completo_ = 0;
		mejora=false;
		vector <bool> repetido(solucion.size(), false);
		bool completo = false;
		...
		while(!mejora && !completo && iter<limite)
		{
			...
			int a = flip(s, repetido);

			if(a!=ultimo_flip_bueno)
			{
				iter++;
				float s_promedio = funcion_objetivo(training, test, s);
...
int main()
{
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		busqueda_local(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc1, submuestra_wdbc2);
		...
		start_timers();
		busqueda_local(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc2, submuestra_wdbc1);
		time = elapsed_time();
		soluciones_promedio_wdbc.push_back(solucion_wdbc_promedio);
		soluciones_wdbc.push_back(solucion_wdbc);
		tiempos_wdbc.push_back(time);
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		busqueda_local(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia1, submuestra_arrhythmia2);
		double time = elapsed_time();
		...
		start_timers();
		busqueda_local(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia2, submuestra_arrhythmia1);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		busqueda_local(solucion_movement_promedio, solucion_movement, submuestra_movement1, submuestra_movement2);
		double time = elapsed_time();
		...
		start_timers();
		busqueda_local(solucion_movement_promedio, solucion_movement, submuestra_movement2, submuestra_movement1);
		time = elapsed_time();
