...
////////////////////////////////////Búsqueda Tabú
void busqueda_tabu(float & mejor_solucion, vector <bool> & solucion, vector <Caracteristicas> & training, 
	vector <Caracteristicas> & test)
{
	...
	int tam_lista = solucion.size()/3;
	vector <bool> s = solucion;
	float s_promedio = mejor_solucion;


	while(iters < limite)
	{

		vector <bool> mejor_vecino;
		float mejor_vecino_promedio=0;
		for(int i = 0 ; i < 30 && iters < limite ; i++)
		{
			bool aceptado = true;
			vector <bool> s_tmp = s;
			int ind = flip(s_tmp);
			float tmp_promedio = funcion_objetivo(training, test, s_tmp);
			iters++;

			for(int j = 0 ; j < lista_tabu.size() && aceptado ; j++)
			{
				if(lista_tabu[j] == ind) aceptado=false;
			}


			if(aceptado)
			{
				if(tmp_promedio > mejor_vecino_promedio)
				{
					mejor_vecino_promedio = tmp_promedio;
					mejor_vecino = s_tmp;
				}

				if(lista_tabu.size() < tam_lista)
				{
					lista_tabu.push_back(ind);
				}else
				{
				...
				if(tmp_promedio > mejor_solucion)
				{
					solucion = s_tmp;
					mejor_solucion = tmp_promedio;
				}
			}else if(tmp_promedio > mejor_solucion)
			{
				mejor_solucion = tmp_promedio;
				solucion = s_tmp;
				mejor_vecino = s_tmp;
		...
		s= mejor_vecino;
		s_promedio = mejor_vecino_promedio;
	}
...
int main()
{
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		busqueda_tabu(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc2, submuestra_wdbc1);
		time = elapsed_time();
		soluciones_promedio_wdbc.push_back(solucion_wdbc_promedio);
		soluciones_wdbc.push_back(solucion_wdbc);
		tiempos_wdbc.push_back(time);
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		busqueda_tabu(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia1, submuestra_arrhythmia2);
		double time = elapsed_time();
		...
		start_timers();
		busqueda_tabu(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia2, submuestra_arrhythmia1);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		busqueda_tabu(solucion_movement_promedio, solucion_movement, submuestra_movement1, submuestra_movement2);
		double time = elapsed_time();
		...
		start_timers();
		busqueda_tabu(solucion_movement_promedio, solucion_movement, submuestra_movement2, submuestra_movement1);
		time = elapsed_time();
