...
*/
		...
		{
			for(int k = 0 ; k < matriz.at(j).size() ; k++)
			{
				cout << matriz[j][k] << " ";
			}
...
*/
		...
		for(int j = 0 ; j < labels.size() ;j++)
		{
			for(int k = 0 ; k < 10 ; k++)
			{
				//float p_c_f = matriz.at(j).at(k)*1.0/conjunto_datos.size()*1.0;
				double p_i_j=0.0;
				int total=0;
				...
				//cout << p_c_f*log2(p_c_f/((n_class.at(j)/labels.size())*p_i_j))<< endl;
				//cout << p_c_f/(( (double) n_class[j]/conjunto_datos.size())*p_i_j) << endl;
				//cout << labels.size() << " ";
...
void SHMMBL(float & mejor_solucion, vector <bool> & solucion, vector <Caracteristicas> & training, vector <float> & heuristic)
{

	generar_solucion_aleatoria(solucion, solucion.size());
	mejor_solucion = funcion_objetivo(training, solucion);
	double max_feromona = mejor_solucion/0.2;
	double min_feromona = mejor_solucion/500.0;
	...
	vector <double> feromona_car(solucion.size(), max_feromona);
	...
	//vector <vector <double> > probs_aux(10); //Un vector auxiliar de probabilidades para cada hormiga
	//UN vector de candidatos (índices) para cada hormiga

	//vector <bool> s_act(solucion.size(), false);
	...
	while(evaluaciones < 15000)
	{
		...
		vector <vector <int> > candidatos(10,candidatos_aux);
		...
		{
			//Hacemos la ruleta
			float aleatorio = Randfloat(0,feromona_n_car_total);
			bool fin = false;
		...
		for(int i = 0 ;  i < n_car_max ; i++)
		{
			for(int j = 0 ; j < 10 ; j++)
			{
				...
				if(n_car_seleccionadas[j] < n_car[j])
				{
					...
					//Hago la regla de transición de SH sin actualización local de feromona
		...
		for(int i = 0 ; i < grafos.size() ; i++)
		{
			...
			if(s_act_valor >= mejor_solucion)
			{
				...
				max_feromona = s_act_valor/0.2;
				min_feromona = s_act_valor/500.0;
			}
		}
		...
		{
			feromona_car[i] = 0.8*feromona_car[i]+solucion[i]*mejor_solucion;

			if(feromona_car[i] > max_feromona) feromona_car[i]=max_feromona;
			else if(feromona_car[i] < min_feromona) feromona_car[i]=min_feromona;
		}
		...
		{
			feromona_n_car[i] = 0.8*feromona_n_car[i]+(mejor_n_car==i+1)*mejor_solucion;
...
int main()
{
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		SHMMBL(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc1, heuristic);
		...
		start_timers();
		SHMMBL(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc2, heuristic);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		SHMMBL(solucion_movement_promedio, solucion_movement, submuestra_movement1, heuristic);
		double time = elapsed_time();
		...
		start_timers();
		SHMMBL(solucion_movement_promedio, solucion_movement, submuestra_movement2, heuristic);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		SHMMBL(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia1, heuristic);
		double time = elapsed_time();
		...
		start_timers();
		SHMMBL(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia2, heuristic);
		time = elapsed_time();
		cout << "hola " << endl;
