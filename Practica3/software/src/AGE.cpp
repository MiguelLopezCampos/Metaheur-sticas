...
void AGE(float & mejor_solucion, vector <bool> & solucion, vector <Caracteristicas> & training)
{
	...
	//Con el array cromosomas disponibles consigo que en pasos posteriores
	//un cromosoma no compita consigo mismo en el torneo
	//de selección
	//También lo uso para que no se mute 2 veces el mismo cromosoma
	...
	//Incremento la i de 2 en 2 ya que en cada
	//iteración se realizan 2 evaluaciones de la función objetivo
	//empiezo en i=30 porque ya se han realizado 30 evaluaciones
	for(int i = 30 ; i <= 15000 ; i+=2)
	{
		//Fase de selección. Realizo 2 concursos.
		//Tengo que controlar que un cromosoma no compita consigo mismo
		vector <bool> padre1;
		vector <bool> padre2;

		vector <int> posibles = cromosomas_disponibles;
		int r1 = Randint(0, posibles.size()-1);
		//Eliminando este entero del vector posibles lo que consigo es que
		//en el siguiente random no pueda salir un índice igual al anterior
		//en posibles tengo los índices posibles del array de población
		posibles.erase(posibles.begin()+r1);
		int r2 = Randint(0, posibles.size()-1);

		posibles = cromosomas_disponibles;
		int r3 = Randint(0, posibles.size()-1);
		posibles.erase(posibles.begin()+r3);
		int r4 = Randint(0, posibles.size()-1);

		posibles = cromosomas_disponibles;
		if(tasas_poblacion[posibles[r1]] >= tasas_poblacion[posibles[r2]]) padre1 = poblacion[posibles[r1]];
		else padre1 = poblacion[posibles[r2]];

		if(tasas_poblacion[posibles[r3]] >= tasas_poblacion[posibles[r4]]) padre2 = poblacion[posibles[r3]];
		else padre2 = poblacion[posibles[r4]];
		...
		if(random1 <= 0.001)
		{
			r1 = Randint(0, padre1.size()-1);
			if(padre1[r1] == true) padre1[r1] = false;
			else padre1[r1] = true;
		}

		if(random2 <= 0.001)
		{
			r2 = Randint(0, padre2.size()-1);
			if(padre2[r2] == true) padre2[r2] = false;
			else padre2[r2] = true;
		}
		...
		float tasa_padre1 = funcion_objetivo(training, padre1);
		float tasa_padre2 = funcion_objetivo(training, padre2);

		vector <float> aux = tasas_poblacion;

		int min1 = minimo(aux);
		aux[min1] = 9999999999.9;
		int min2 = minimo(aux);


		//Si el padre 2 es mejor que el padre 1, los intercambio
		if(tasa_padre1 <= tasa_padre2)
		{
			float aux_tasa = tasa_padre1;
			vector <bool> aux_padre = padre1;
			padre1=padre2;
			tasa_padre1=tasa_padre2;
			padre2 = aux_padre;
			tasa_padre2 = aux_tasa;
		}


		if(tasa_padre1 >= tasas_poblacion[min1])
		{
			tasas_poblacion[min1] = tasa_padre1;
			poblacion[min1] = padre1;

			if(tasa_padre2 >= tasas_poblacion[min2])
			{
				tasas_poblacion[min2] = tasa_padre2;
				poblacion[min2] = padre2;
			}
		}else if(tasa_padre1 >= tasas_poblacion[min2])
		{
...
int main()
{
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		AGE(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc2);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		AGE(solucion_movement_promedio, solucion_movement, submuestra_movement1);
		double time = elapsed_time();
		...
		start_timers();
		AGE(solucion_movement_promedio, solucion_movement, submuestra_movement2);
		time = elapsed_time();
...
*/
	...
	for(int i = 2 ; i <= 5 ; i++)
	{
		...
		start_timers();
		AGE(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia1);
		double time = elapsed_time();
		...
		start_timers();
		AGE(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia2);
		time = elapsed_time();
		cout << "hola " << endl;
