...
void mutar(vector <bool> & s)
{
	...
	for(int i = 0 ; i < (int) 0.1*s.size() ; i++)
	{
		int r = Randint(0, aux.size()-1);

		if(s[aux[r]] == false) s[aux[r]]=true;
		else s[aux[r]]=false;
...
///////////////////////////////////ILS
void ILS(float & mejor_solucion, vector <bool> & solucion, vector <Caracteristicas> & training)
{
...
/*	cout << "\n\nWDBC\n\n";
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		ILS(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc1);
		...
		start_timers();
		ILS(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc2);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		ILS(solucion_movement_promedio, solucion_movement, submuestra_movement1);
		double time = elapsed_time();
		...
		start_timers();
		ILS(solucion_movement_promedio, solucion_movement, submuestra_movement2);
		time = elapsed_time();
...
*/
	...
	for(int i = 3 ; i <= 5 ; i++)
	{
		...
		start_timers();
		ILS(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia1);
		double time = elapsed_time();
		...
		start_timers();
		ILS(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia2);
		time = elapsed_time();
		cout << "hola " << endl;
