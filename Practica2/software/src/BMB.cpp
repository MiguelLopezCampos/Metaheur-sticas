...
////////////////////////////////////BMB
void BMB(float & mejor_solucion, vector <bool> & solucion, vector <Caracteristicas> & training)
{
	vector <bool> s_act = solucion;
	generar_solucion_aleatoria(s_act, s_act.size());
	float s_act_valor = funcion_objetivo(training, s_act);

	solucion = s_act;
	mejor_solucion = s_act_valor;

	//Como el criterio de parada es repetir la BL 25 veces, lo hago en un bucle for
	for(int i = 1 ; i <= 25 ; i++)
	{
 
		busqueda_local(s_act_valor, s_act, training);
		if(mejor_solucion < s_act_valor)
		{
			mejor_solucion = s_act_valor;
			solucion = s_act;
		}

		//Genero nueva solución aleatoria
...
int main()
{
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		BMB(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc1);
		...
		start_timers();
		BMB(solucion_wdbc_promedio, solucion_wdbc, submuestra_wdbc2);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 5 ; i++)
	{
		...
		start_timers();
		BMB(solucion_movement_promedio, solucion_movement, submuestra_movement1);
		double time = elapsed_time();
		...
		start_timers();
		BMB(solucion_movement_promedio, solucion_movement, submuestra_movement2);
		time = elapsed_time();
	...
	for(int i = 1 ; i <= 2 ; i++)
	{
		...
		start_timers();
		BMB(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia1);
		double time = elapsed_time();
		...
		start_timers();
		BMB(solucion_arrhytmia_promedio, solucion_arrhytmia, submuestra_arrhythmia2);
		time = elapsed_time();
		cout << "hola " << endl;
