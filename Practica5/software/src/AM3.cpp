...
void AGG(float & mejor_solucion, vector <bool> & solucion, vector <Caracteristicas> & training)
{
	...
	while(iters < 15000)
	{
		...
		if(generacion == 10)
		{
			//Calculo los 10 mejores cromosomas
			vector <float> tasas_aux = tasas_poblacion;
			vector <int> indices_mejores;

			for(int j = 0 ; j < 0.1*poblacion.size() ; j++)
			{
				int m = maximo(tasas_aux);
				indices_mejores.push_back(m);
				tasas_aux[m] = -9999999.9;
			}


			for(int j = 0 ; j < indices_mejores.size() ; j++)
			{
				bool mejora=false;
				vector <int> repetido;
				bool completo = false;
				repetido.resize(poblacion[indices_mejores[j]].size());

				//Inicialización de repetido
				//Repetido se inicializa con todos los índices posibles
				//del vector solución.
				for(int i = 0 ; i < poblacion[indices_mejores[j]].size() ; i++)
				{
					repetido.at(i) = i;
				}
				...
				{
					vector <bool> s = poblacion[indices_mejores[j]];
					...
					{
						poblacion[indices_mejores[j]] = s;
						tasas_poblacion[indices_mejores[j]] =s_promedio;
						mejora=true;
					}
